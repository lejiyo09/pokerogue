...
**최종 결정(사용자 확정): D. PvE 파티 변경 시 동기화.** 런 클리어를 기다리지 않고, 파티 구성이 바뀔 때마다(포획/부화/교환 등으로 새 개체가 파티에 들어올 때) 즉시 Global Collection과 동기화한다. A(포획 즉시 입고)보다 더 넓은 개념 — "포획"뿐 아니라 파티에 새 개체가 편입되는 모든 경로를 다 잡는다.

**구현 지점**: 파티에 개체가 추가될 수 있는 경로(포획 `attempt-capture-phase.ts`, 부화 `egg-hatch-phase.ts`/`egg-lapse-phase.ts`, 미스터리 인카운터 보상 등)를 전부 개별적으로 훅걸기보다, **기존 세이브 파이프라인의 단일 관문인 `GameData.saveAll()`(`game-data.ts:1204`)에 얹는 편이 안전하다.** 이 함수는 이미 `getSessionSaveData()`로 현재 파티 전체를 직렬화해 서버로 보내는, 게임이 주기적으로(그리고 각종 이벤트 이후) 자동 호출하는 유일한 저장 관문이다. 개별 이벤트마다 새로 훅을 심는 대신, 이 지점에서 "직전에 동기화한 파티원 집합"과 "현재 파티"를 개체 UID 기준으로 비교해 **새로 추가된 개체만 Global Collection에 POST**하면 부화든 교환이든 앞으로 추가될 새 파티 편입 경로든 빠짐없이 잡힌다 — 매 경로를 놓치는 실수를 구조적으로 방지한다.

**멱등성 주의**: "파티 변경 시 동기화"를 "매번 새 레코드 생성"으로 구현하면 안 된다. 같은 개체가 레벨업/기술 습득/IV 변화 등으로 여러 번 `saveAll()`을 통과하게 되는데, 이미 입고된 개체(UID로 식별)는 **덮어쓰기(upsert)**해야 한다 — 그렇지 않으면 "용량 무제한" 정책(2.4절 결정) 아래에서 사용량이 실제 개체 수가 아니라 저장 이벤트 횟수에 비례해 폭증한다.
...
- 저장 용량: **사용자 확정 — 계정당 별도 상한 없음(무제한).** 다만 `PokemonData` 하나가 결코 작지 않은데(모든 필드 포함), "용량 무제한" 정책 자체는 유지하되 **전송/조회 방식은 무제한과 별개의 구현 문제**다. 헤비 유저가 수백~수천 마리를 모으면: (1) `GET /pvp/collection` 한 번에 전부 내려받는 방식은 응답이 비대해지므로 페이지네이션이 필요하고, (2) 2.2절의 `saveAll()` 훅에서 매번 컬렉션 전체가 아니라 **새로 추가된 개체만** POST하는 한(위 멱등성 설계) 요청 1건은 항상 작게 유지되므로 서버의 `MaxBytesReader`(이번 세션에서 8MiB로 설정) 한도와 충돌하지 않는다 — "계정 총량 무제한"과 "요청 1건은 작게 유지" 둘 다 만족 가능.
...
**최종 결정(사용자 확정)**: 리전폼은 **별개 종으로 취급한다** — 기본종과 묶지 않는다. 신규 매핑 테이블을 만들 필요가 없어지므로 구현이 가장 단순해지고, 이 프로젝트의 다른 모든 시스템(도감, 스타터 코스트, 진화)이 이미 리전폼을 별개 종으로 취급하고 있는 기존 관례와도 정확히 일치한다. 즉 **② 자체가 추가 규칙이 아니라 "아무것도 안 하면(SpeciesId 값을 그대로 비교하면) 된다"**로 귀결된다 — 알로라 나옹과 기본 나옹은 이미 다른 `SpeciesId`이므로 별다른 처리 없이도 서로 다른 종으로 판정된다.
...
기존 아이템 정의(`modifierTypes` 레지스트리, `src/modifier/modifier-type.ts` — 이번 세션 초반 치트 계정 아이템 지급 기능에서 다뤘던 바로 그 구조)에 **이미 카테고리 구분이 어느 정도 존재한다**(예: `PokemonHeldItemModifierType`처럼 특정 포켓몬에 귀속되는 타입 vs 계정 전역 `ModifierType`). PvE 전용 로그라이크 효과(예: Endless 전용 누적 버프)는 대개 `ModifierType`이 아니라 `Battle`/`Arena` 상태나 챌린지 로직에 직접 박혀있을 가능성이 높다 — 즉 **"held item으로 표현되는 것"과 "런 상태로 표현되는 것"의 경계가 이미 어느 정도 카테고리 분리 역할을 한다.**
...
이 구조는 "처음엔 다 false, 나중에 조건부로 true"라는 사용자의 요구사항을 설정 값 하나 바꾸는 것으로 만족시키므로 추가 설계가 필요 없다.

**최종 결정(사용자 확정) — 해금 일정은 랭크/승수가 아니라 달력 기준이다:**

| 시점 | 활성화되는 기믹 |
|---|---|
| 10월 | `allowMega = true`, `allowZMove = true` |
| 11월 | `allowTerastal = true` 추가 |
| 12월 | `allowGigantamax = true` 추가 (10~12월 항목 전부 누적 활성) |

`PvpRuleset`이 서버가 배틀 시작 시 내려주는 값인 만큼, 이 일정은 클라이언트가 아니라 **서버(`rogueserver` 또는 `pvp-server`)가 현재 날짜를 기준으로 매 배틀마다 `PvpRuleset`을 계산해 내려주는 방식**으로 구현해야 한다 — 클라이언트의 로컬 시계값을 믿고 기믹 해금 여부를 판단하면 시계를 조작해 조기 해금하는 것이 가능해지므로, 반드시 서버 측 날짜 기준이어야 한다(12장 보안/안티치트와 같은 원칙).
...
모든 항목이 확정됐다 — 아래는 각 결정의 근거로 돌아갈 수 있도록 남겨두는 이력이다.

- ~~0단계(세이브 슬롯 재사용 MVP)를 실제로 먼저 만들지~~ → **판단(7절): 건너뛴다.** 2.2절의 확정된 입고 정책이 Global Collection의 존재를 전제하고, 아이템/기믹 결정까지 구체적이라 임시 MVP를 거칠 이유가 없다.
- ~~2.2절의 입고 시점(A/B/C)~~ → **사용자 확정: D(PvE 파티 변경 시 동기화)**. `GameData.saveAll()` 훅 + UID 기준 upsert로 구현(2.2절).
- ~~3.2절의 정확한 PvP 허용 아이템 allow-list~~ → **사용자 확정: VGC/Pokémon Champions 룰셋 기준**. 정확한 레지스트리 매핑은 구현 단계 작업(3.2절).
- ~~5장 기믹의 구체적 해금 조건~~ → **사용자 확정: 10월 메가/Z-Move → 11월 테라스탈 → 12월 기가맥스**, 서버 날짜 기준으로 계산(5장).
- ~~Global Collection의 용량 상한~~ → **사용자 확정: 무제한**. 단 전송은 "새 개체만 upsert"로 요청 크기를 작게 유지(2.3절).
